...
async function runForUser(label, email, password) {
  ...
  const row = {
    name: `${label} Test Staff`,
    role: 'Teacher',
    department: 'QA',
    email: `${label.toLowerCase()}.qa@example.com`,
    phone: '0000000000',
  };

  const insert = await client.from('staff').insert(row).select('*');
  ...
  const update = await client.from('staff').update({ department: 'QA Updated' }).eq('id', id).select('*');
  ...
  const del = await client.from('staff').delete().eq('id', id).select('*');
